...
if [ -n "${BASH_VERSION}" ]; then
    ...
    function custom_build_prompt {
        ...
        if [[ $is_a_git_repo == true ]]; then
            ...
            prompt+=$(enrich_append $is_a_git_repo $repo_status_symbol "${left_side}")
            ...
            if [[ ${omg_condensed} == false ]]; then
                ...
                prompt+=$(enrich_append $has_renames $omg_has_renames_symbol "${left_side}")
            fi
		...
		prompt+=$(enrich_append $has_deletions_cached $omg_has_cached_deletions_symbol "${left_side}")
            ...
            prompt="${prompt} ${right_icon}${omg_arrow_symbol} ${right_side}"

            if [[ $detached == true ]]; then
                if [[ "${action}" = "rebase" ]]; then
                    prompt+=$(enrich_append $detached $omg_rebase_interactive_symbol "${right_icon}")
                elif [[ "${action}" = "bisect" ]] && [[ "${bisect_steps}" = "0" ]]; then
                    prompt+=$(enrich_append $detached "$omg_bisect_done_symbol" "${right_icon}")
                elif [[ "${action}" = "bisect" ]] && [[ "${bisect_steps}" = "~0" ]]; then
                    prompt+=$(enrich_append $detached "${bisect_tested}/${bisect_total} $omg_bisect_close_symbol  ${bisect_steps}" "${right_icon}")
                elif [[ "${action}" = "bisect" ]]; then
                    prompt+=$(enrich_append $detached "${bisect_tested}/${bisect_total} $omg_bisect_symbol  ${bisect_steps}" "${right_icon}")
                else
            ...
            for tag in ${tags_at_current_commit}; do
                prompt+=$(enrich_append true "${omg_is_on_a_tag_symbol} ${tag}" "${right_side}")
            done
            prompt+=$(enrich_append ${is_virtualenv} "${omg_is_virtualenv_symbol}  ${virtualenv}" "${right_icon}")
